# Implementation Plan — Display Density (DPI) Overhaul & Safety Guard System (Updated to 120 DPI presets)
...
- **Presets focus heavily on scaling down the UI with 120 DPI as the ultra-compact baseline**: Desktop shell users seek high information density. We set the lowest preset to **120 DPI** (giving a full 1440dp canvas on 1080p screens!), followed by **160 DPI** (Android baseline mdpi), 240, 320, 360, Default, and 480 DPI presets.
...
1. **Interactive Presets & Custom Input UI (Skewed for Scaling Down)**:
   ...
   - Presets focus on **scaling down** (lower DPI values to pack more content on screen) relative to the default density:
     - **`120 DPI`** (Ultra Compact — gives a full 1440dp canvas on 1080p screens!)
     - **`160 DPI`** (Desktop baseline — gives a full 1080dp canvas on 1080p screens!)
...
2. **Built-in Screen Safety Interceptor & Dynamic Bounds**:
   ...
   - **Dynamic Safety Limits**: Rather than hardcoding bounds (like 240–600), we calculate bounds dynamically based on physical screen parameters:
     ...
     - **Unified Dynamic Limits**:
       - `minAllowedDpi = maxOf(minDpiFromDp.toInt(), (physicalDensity * 0.5).toInt()).coerceAtLeast(120)`
...
3. **Build Scrolling Row of Presets**:
   - Construct a preset list: `val defaultDpis = remember(physicalDensity) { mutableListOf(120, 160, 240, 320, 360, 480).apply { if (!contains(physicalDensity)) add(physicalDensity) }.sorted() }`.
...
1. **120 DPI Preset**:
   ...
   - Verify that the first preset chip is **`120 DPI`** (labeled as `120`).
   - Click it, confirm the warnings. Verify it scales the phone UI down perfectly, transforming the display into a spacious, standard-sized 1440dp wide canvas.
   - Verify that `160 DPI` chip is available between `120 DPI` and `240 DPI`.
